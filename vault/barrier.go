...
const (
	...
	// path from key N to N+1. They cannot just use the main key because
	// in the event of a rekey, that main key can no longer decrypt the keyring.
	...
	// mainKeyPath is the location of the main key. This is encrypted
	...
	// have the old main key. This key can be decrypted if you have the
	// keyring to discover the new main key. The new main key is then
	// used to reload the keyring itself.
	mainKeyPath = "core/main"

	// shamirKekPath is used with Shamir in v1.3+ to store a copy of the
	// unseal key behind the barrier.  As with mainKeyPath this is primarily
...
type SecurityBarrier interface {
	...
	// and makes use of the given main key.  When sealKey is provided
	// it's because we're using a new-style Shamir seal, and mainKey
	// is to be stored using sealKey to encrypt it.
	Initialize(ctx context.Context, mainKey []byte, sealKey []byte, random io.Reader) error
	...
	// SetMasterKey is used to directly set a new main key. This is used in
	// replicated scenarios due to the chicken and egg problem of reloading the
	// keyring from disk before we have the main key to decrypt it.
