		...
		},
		{
			...
			Fields: map[string]*framework.FieldSchema{
				"key": &framework.FieldSchema{
					Type:        framework.TypeString,
					Description: "Specifies a single main key share.",
				},
			...
			Operations: map[logical.Operation]framework.OperationHandler{
				logical.UpdateOperation: &framework.PathOperation{
					Summary:     "Enter a single main key share to progress the root generation attempt.",
					Description: "If the threshold number of main key shares is reached, Vault will complete the root generation and issue the new token. Otherwise, this API must be called multiple times until that threshold is met. The attempt nonce must be provided with each call.",
				},
			},
...

		{
			...
			Fields: map[string]*framework.FieldSchema{
				...
				"secret_shares": &framework.FieldSchema{
					Type:        framework.TypeInt,
					Description: "Specifies the number of shares to split the main key into.",
				},
				"secret_threshold": &framework.FieldSchema{
					Type:        framework.TypeInt,
					Description: "Specifies the number of shares required to reconstruct the main key. This must be less than or equal secret_shares. If using Vault HSM with auto-unsealing, this value must be the same as `secret_shares`.",
				},
...
func (b *SystemBackend) rekeyPaths() []*framework.Path {
	return []*framework.Path{
		...
		},
		{
			...
			Operations: map[logical.Operation]framework.OperationHandler{
				logical.UpdateOperation: &framework.PathOperation{
					Summary: "Enter a single main key share to progress the rekey of the Vault.",
				},
			},
		},
		{
			...
			Fields: map[string]*framework.FieldSchema{
				"key": &framework.FieldSchema{
					Type:        framework.TypeString,
					Description: "Specifies a single main share key from the new set of shares.",
				},
				"nonce": &framework.FieldSchema{
					Type:        framework.TypeString,
...

		{
			...
			Fields: map[string]*framework.FieldSchema{
				"key": &framework.FieldSchema{
					Type:        framework.TypeString,
					Description: "Specifies a single main key share. This is required unless reset is true.",
				},
