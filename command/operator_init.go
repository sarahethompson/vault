...
Usage: vault operator init [options]
  ...
  During initialization, Vault generates an in-memory main key and applies
  Shamir's secret sharing algorithm to disassemble that main key into a
  configuration number of key shares such that a configurable subset of those
  key shares must come together to regenerate the main key. These keys are
...
func (c *OperatorInitCommand) Flags() *FlagSets {
	...
	f.IntVar(&IntVar{
		...
		Usage: "Number of key shares to split the generated main key into. " +
	...
	f.IntVar(&IntVar{
		...
		Completion: complete.PredictAnything,
		Usage: "Number of key shares required to reconstruct the main key. " +
...
func (c *OperatorInitCommand) init(client *api.Client, req *api.InitRequest) int {
	...
	if len(resp.Keys) > 0 {
		...
		c.UI.Output("")
		c.UI.Output(wrapAtLength(fmt.Sprintf(
			"Vault does not store the generated main key. Without at least %d "+
				"key to reconstruct the main key, Vault will remain permanently "+
				"sealed!",
			req.SecretThreshold)))
