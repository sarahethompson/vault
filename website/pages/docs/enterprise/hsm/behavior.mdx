...
Vault usually generates a main key and splits it using [Shamir's Secret
...
When using an HSM, Vault instead stores the main key, encrypted by the HSM,
...
to not split the main key, so that the single key share is itself the main
...
Both rekeying the main key and rotation of the underlying data
