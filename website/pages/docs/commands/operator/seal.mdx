...
description: >-
  ...
  server discards its in-memory main key to unlock the data, so it is
...
stop responding to any operations until it is unsealed. When sealed, the Vault
server discards its in-memory main key to unlock the data, so it is physically
...
Users will have to re-enter their portions of the main key again.
