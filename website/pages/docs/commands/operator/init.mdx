...
shares must come together to regenerate the main key. These keys are often
...
- `-key-shares` `(int: 5)` - Number of key shares to split the generated main
...
- `-key-threshold` `(int: 3)` - Number of key shares required to reconstruct the
  main key. This must be less than or equal to -key-shares. This is aliased as
  `-t`.
