...
[high-level architecture](/docs/internals/architecture) before learning about key rotation.
...
to split the main key into 5 shares, any 3 of which are required to reconstruct the main
key. The main key is used to protect the encryption key, which is ultimately used to protect
...
To support key rotation, we need to support changing the unseal keys, main key, and the
backend encryption key. We split this into two separate operations, `rekey` and `rotate`.

The `rekey` operation is used to generate a new main key. When this is being done,
...
performing a rekey and invalidating the existing main key.
...
must be provided until the threshold is met. Once met, Vault will generate the new main
key, perform the splitting, and re-encrypt the encryption key with the new main key.
