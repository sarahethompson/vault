...
_Unsealing_ is the process of obtaining the plaintext main key necessary to
...
encryption key known as the _main key_.

Therefore, to decrypt the data, Vault must decrypt the encryption key
which requires the main key. Unsealing is the process of getting access to
this main key.  The main key is stored alongside all other Vault data,
...
the keyring is encrypted by the main key; and the main key is encrypted by
the unseal key.
...
reconstruct the unseal key, which is then used to decrypt the main key.
...
on multiple computers and it will work. This allows each shard of the main
...
There is also an API to seal the Vault. This will throw away the main
...
to decrypt the main key Vault read from storage.
