...
This endpoint is used to enter a single main key share to progress the root
generation attempt. If the threshold number of main key shares is reached,
...
- `key` `(string: <required>)` – Specifies a single main key share.
