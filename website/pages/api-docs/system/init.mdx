...
- `secret_shares` `(int: <required>)` – Specifies the number of shares to
  split the main key into.

- `secret_threshold` `(int: <required>)` – Specifies the number of shares
  required to reconstruct the main key. This must be less than or equal
...
provided) main keys, base 64 encoded main keys and initial root token:
