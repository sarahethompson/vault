...
can be provided to rekey the main key since no unseal keys are available. The
...
- `secret_threshold` `(int: <required>)` – Specifies the number of shares
  required to reconstruct the main key. This must be less than or equal to
  `secret_shares`.
...
- `require_verification` `(bool: false)` – This turns on verification
  ...
  with the current unseal keys, the new unseal keys are returned but the main
  key is not actually rotated. The new keys must be provided to authorize the
  actual rotation of the main key. This ensures that the new keys have been
...
This endpoint is used to enter a single main key share to progress the rekey
of the Vault. If the threshold number of main key shares is reached, Vault
...
- `key` `(string: <required>)` – Specifies a single main share key.
...
Vault will complete the rekey by performing the actual rotation of the main
...
- `key` `(string: <required>)` – Specifies a single main share key from the
  new set of shares.
