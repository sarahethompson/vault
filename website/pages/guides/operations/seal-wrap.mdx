...
- **Master Key Wrapping**: Vault protects its main key by transiting it through
  the HSM for encryption rather than splitting into key shares
- **Automatic Unsealing**: Vault stores its encrypted main key in storage,
...
Vault pulls its encrypted main key from storage and transit it through the
HSM for decryption via **PKCS \#11 API**. Once the main key is decrypted,
Vault uses the main key to decrypt the encryption key to resume with Vault
operations.
...
There is only a single main key created which is encrypted by the HSM using
PKCS \#11, and then placed in the storage. When Vault needs to be unsealed, it
grabs the HSM encrypted main key from the storage, round trips it through the
HSM to decrypt the main key.
...
stored key shares, the main key, the keyring, and more. When working with the
