...
When a Vault server is first initialized, Vault generates a main key and
immediately splits this main key into a series of key shares following
Shamir's Secret Sharing Algorithm. Vault never stores the main key, therefore,
the only way to retrieve the main key is to have a quorum of unseal keys
re-generate it.

The main key is used to decrypt the underlying encryption key. Vault uses the
...
In some cases, you may want to re-generate the main key and key shares. Here
are a few examples:

- Someone joins or leaves the organization
- Security wants to change the number of shares or threshold of shares
- Compliance mandates the main key be rotated at a regular interval

In addition to rekeying the main key, there may be an independent desire to
...
generating a new main key and applying Shamir's algorithm is called
...
Vault does not store the main key. Without at least 2 keys,
