...
model](/docs/internals/security) and focus on defense in depth.
...
- **Disable Swap**. Vault encrypts data in transit and at rest, however it must
  ...
  automatically](/docs/configuration#disable_mlock), but disabling
...
- **Enable Auditing**. Vault supports several auditing backends. Enabling
  ...
  hash](/docs/audit) any sensitive data, but access should still be
...
- **Restrict Storage Access**. Vault encrypts all data at rest, regardless of
  ...
  with arbitrary control](/docs/internals/security) can cause data
...
- **Docker Containers**. To leverage the ["memory lock"](/docs/configuration#disable_mlock)
